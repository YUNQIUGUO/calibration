...
class DataRetriever:
    def __init__(self, images_folder,augmented_model_path='augmented_model.onnx'):
        self.preprocess_func = preprocess_func
        self.images_folder = images_folder
        self.augmented_model_path = augmented_model_path
        self.counter = 0
        self.enum_data_list = enumerate({})
    ...
    def get_next(self):
        if counter==0:
            ...
            img_array = self.preprocess_func(self.images_folder, height, width, size_limit=0)
            input_name = session.get_inputs()[0].name
            self.enum_data_list = enumerate({input_name: img_array})
        return self.enum_data_list.next()
